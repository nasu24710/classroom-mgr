...
class LectureRoomManagementInformation < Data.define(:date, :day_of_the_week, :term, :periods, :room_name, :subject, :user, :comment)
    ...
    def self.room_names_conflict?(room_name, other_room_name)
        return true if room_name == other_room_name

        full_lecture_room_name?(room_name) && lecture_room_name?(other_room_name) ||
            full_lecture_room_name?(other_room_name) && lecture_room_name?(room_name)
    end

    def self.lecture_room_name?(room_name)
        room_name.is_a?(String) && room_name.match?(LECTURE_ROOM_NAME_PATTERN)
    end

    def self.full_lecture_room_name?(room_name)
        room_name == FULL_LECTURE_ROOM_NAME
    end
